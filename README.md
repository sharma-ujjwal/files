				...
				<p:panel id="accessListPanel" styleClass="accessListPanel" style="padding: 0px !important;">
					...
					<p:dataTable id="privilegeTable"
						...
						<p:column id="sourceNameColumn" headerText="Source Name"
								  ...
								  style="width: 10%; background-color: #{rowStatus % 2 != 0 ? (accessListBean.review.SODReview ? '#FFE5E6' : '#3d80df') : '#fffff'}!important;">
							<h:outputText value="#{priv.sourceName}"/>
						</p:column>
						<p:column id="privilegeDescriptionColumn" headerText="Privilege Description"
								  headerStyle="background-color: #{accessListBean.headerStyleColor} !important;"
								  style="width: 20%; background-color: #{rowStatus % 2 != 0 ? (accessListBean.review.SODReview ? '#FFE5E6' : '#3d80df') : '#fffff'}!important;">
						...
						<p:column id="privilegeIdColumn" headerText="Privilege ID"
								  headerStyle="background-color: #{accessListBean.headerStyleColor} !important;"
								  style="width: 60%; background-color: #{rowStatus % 2 != 0 ? (accessListBean.review.SODReview ? '#FFE5E6' : '#3d80df') : '#fffff'}!important;">
							<h:outputText value="#{priv.privilegeComment}"/>
						</p:column>
						<p:column id="soxConcernColumn" headerText="SOX Concern" rendered="#{accessListBean.review.notManagerReview}"
								  headerStyle="background-color: #{accessListBean.headerStyleColor} !important;"
								  style="width: 5%; background-color: #{rowStatus % 2 != 0 ? (accessListBean.review.SODReview ? '#FFE5E6' : '#3d80df') : '#fffff'}!important;">
							<h:outputText value="#{priv.soxConcern.soxConcernCode}" style="text-align:center;"/>
						</p:column>
						<p:column style="text-align:center; width: 10%;
										 background-color: #{rowStatus % 2 != 0 ? (accessListBean.review.SODReview ? '#FFE5E6' : '#3d80df') : '#fffff'}!important;"
								  headerStyle="background-color: #{accessListBean.headerStyleColor} !important;"
